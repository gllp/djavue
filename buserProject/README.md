...
```bash
dk bash  # starts bash inside "buser_project" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and buser_project.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `buser_project` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The buser_project container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv buser_project -p python3  # creates a python3 virtualenv
